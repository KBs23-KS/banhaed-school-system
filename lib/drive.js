...
export async function downloadBuffer(fileId){ const d=client(); const meta=await d.files.get({fileId,fields:"id,name,mimeType,size"}); const r=await d.files.get({fileId,alt:"media"},{responseType:"arraybuffer"}); return { buffer:Buffer.from(r.data), ...meta.data }; }
